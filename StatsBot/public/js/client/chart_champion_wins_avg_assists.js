function draw_chart_champion_wins_avg_assists(ranked_data, version) {

    var data = new google.visualization.DataTable();
    data.addColumn('number', 'Pick Rate'); // h first
    data.addColumn('number', 'Avg Assists'); // v second
    data.addColumn({ type: 'string', role: 'tooltip' });
    data.addRows(ranked_data);

    var options = {
        title: 'Win Rate vs. Average Assists per Game',
        hAxis: {
    ...
    var chart = new google.visualization.ScatterChart(document.getElementById('chart_champion_wins_avg_assists'));
    ...
    function place_markers(dataTable) { // google charts create SVG elements with object properity 'circle'. 
        ...
        for (var i = 0; i < ranked_data.length; i++) {
            ...
            $('#chart_champion_wins_avg_assists').append(img);
            img.src = link;
            img.height = 16;
            img.width = 16;
            img.style = (
